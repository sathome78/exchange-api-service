...
public enum RefillStatusEnum implements InvoiceStatus {
  X_STATE(0) {
    ...
    public void initSchema(Map<InvoiceActionTypeEnum, InvoiceStatus> schemaMap) {
      schemaMap.put(InvoiceActionTypeEnum.CREATE_BY_USER, CREATED_USER);
      schemaMap.put(InvoiceActionTypeEnum.CREATE_BY_FACT, CREATED_BY_FACT);
    }
  },
  CREATED_USER(1) {
    @Override
    public void initSchema(Map<InvoiceActionTypeEnum, InvoiceStatus> schemaMap) {
      schemaMap.put(InvoiceActionTypeEnum.PUT_FOR_CONFIRM_USER, WAITING_CONFIRMATION_USER);
      schemaMap.put(InvoiceActionTypeEnum.PUT_FOR_PENDING, ON_PENDING);
    }
  },
  CREATED_BY_FACT(2) {
    @Override
  ...
  public InvoiceActionTypeEnum getStartAction(Merchant merchant) {
    if (merchant.getProcessType() == MerchantProcessType.INVOICE) {
      return InvoiceActionTypeEnum.PUT_FOR_CONFIRM_USER;
    } else {
      return InvoiceActionTypeEnum.PUT_FOR_PENDING;
    }
  }
